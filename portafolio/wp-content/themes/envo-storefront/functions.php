...
if (!function_exists('envo_storefront_generate_construct_footer')) :
    ...
    function envo_storefront_generate_construct_footer() {
        ...
        <div class="footer-credits-text text-center">
            ...
            printf(esc_html__('Dersarrollado por %s', 'envo-storefront'), '<a href="' . esc_url(__('https://wordpress.org/', 'envo-storefront')) . '">' . esc_html__('Soluciones TI PWD K-loz', 'envo-storefront') . '</a>');
            ?>
            <span class="sep"> | </span>
            <?php
            /* translators: %1$s: Envo Storefront theme name (do not translate) with envothemes.com URL */
            printf(esc_html__('Tienda Virtual: %1$s', 'envo-storefront'), '<a href="' . esc_url('https://envothemes.com/free-envo-storefront/') . '">' . esc_html_x('Nutrición Virtual', 'Theme name, do not translate', 'envo-storefront') . '</a>');
            ?>
        </div> 
        <?php
    }
